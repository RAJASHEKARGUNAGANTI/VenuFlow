  ...
  if (isLoading) return (
    <div className="space-y-5 max-w-4xl">
      <div className="flex items-center gap-3">
        ...
        <div><Skeleton className="h-6 w-40" /><Skeleton className="h-4 w-24 mt-1" /></div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <Card key={i}><CardContent className="pt-4 pb-4"><Skeleton className="h-3 w-16 mb-2" /><Skeleton className="h-4 w-28" /><Skeleton className="h-3 w-20 mt-1" /></CardContent></Card>
        ))}
      </div>
      <Card><CardContent className="pt-4 pb-4 space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="p-3 bg-muted/40 rounded-lg text-center"><Skeleton className="h-3 w-20 mx-auto mb-2" /><Skeleton className="h-5 w-24 mx-auto" /></div>
          ))}
        </div>
